...
set dist(800) 3.48341e-12
set dist(900) 2.17468e-12
set dist(1000) 1.42681e-12
...
################################################PARALLEL FLOW
for {set i 0} {$i < [expr $num_parallel_flow/2] } {incr i} {
	set udp_node $i
	set null_node [expr (($i+$num_col))%($num_row*$num_col)];#CHNG
...
for {set i [expr $num_parallel_flow/2]} {$i < $num_parallel_flow } {incr i} {
	set udp_node $i
	set null_node [expr ($num_row*$num_col)-1-$i];#CHNG
	$ns attach-agent $node_($udp_node) $udp_($i)
  	$ns attach-agent $node_($null_node) $null_($i)
	puts -nonewline $topofile "PARALLEL: Src: $udp_node Dest: $null_node\n"
}
